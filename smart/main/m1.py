        ...
        if product:
            ...
            while True:
                ...
                if abs(current_weight - expected_new_weight) <= THRESHOLD:
                    ...
                    total_added_weight -= expected_weight  # Only update when correct weight is removed
                    stop_sound()
                    break  # Exit only when the correct weight is removed
                else:
                    if not pygame.mixer.music.get_busy():  # Restart alarm if it's stopped
                        play_sound("signal.mp3")
